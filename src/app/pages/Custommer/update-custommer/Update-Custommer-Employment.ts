...
@Component({
  selector: 'Update-custommer-employment',
  template: `
...
export class UpdateCustommerEmploymentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getEmploymentInfo(); // Fetch employment info after form initialization
  }

  // Fetch Employment Info
  getEmploymentInfo(): void {
    this.route.paramMap.subscribe(params => {
      const customerIdStr = params.get('id');
      const customerId = customerIdStr ? parseInt(customerIdStr, 10) : null;
  
      if (customerId === null) {
        console.error('Invalid customer ID from URL');
        return;
      }
  
      console.log('Editing Employment Info for Customer ID:', customerId);
